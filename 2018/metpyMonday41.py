...
def plot_dataset(dataset):
	...
	data_var = ds.variables['AOD']
	x = ds.variables['x'][:]
	y = ds.variables['y'][:]
	proj_var = ds.variables[data_var.grid_mapping]
	
	# Create a Globe specifying a spherical earth with the correct radius
	globe = ccrs.Globe(ellipse = 'sphere', semimajor_axis = proj_var.semi_major, semiminor_axis = proj_var.semi_minor)
	
	proj = ccrs.LambertConformal(central_longitude = proj_var.longitude_of_central_meridian, central_latitude = proj_var.latitude_of_projection_origin, standard_parallels = [proj_var.standard_parallel], globe = globe)
	...
	text_time = ax.text(0.01, 0.01, timestamp.strftime('%d %B %Y %H%MZ'), horizontalaligment = 'left', transform = ax.transAxes, color = 'white', fontsize = 100, weight = 'bold')
